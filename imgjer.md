...
**Scenario:** Adding USDC to Noble Testnet

**Source Chain:** Noble (mainnet) - NOT Ethereum (origin)
**Why:** Testnet should sync with mainnet version
...
**Solution:** Add image references to chain.json (not necessarily assetlist.json)
...
**Note:** Image URIs can go in `chain.json` (for chain logos) or `assetlist.json` (for token logos), depending on what you're adding.
...
**Maintainer Feedback:** Reviewed by JeremyParish69
